...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="marc:record">
        ...
        <xsl:variable name="OPACResultsMaxItems"
            select="number(marc:sysprefs/marc:syspref[@name='OPACResultsMaxItems']+0)" />
        <xsl:variable name="OPACResultsMaxItemsUnavailable"
            select="number(marc:sysprefs/marc:syspref[@name='OPACResultsMaxItemsUnavailable']+0)" />
        <xsl:variable name="OPACResultsUnavailableGroupingBy"
            select="marc:sysprefs/marc:syspref[@name='OPACResultsUnavailableGroupingBy']" />
        ...
        <xsl:variable name="OPACResultsLibrary"
            select="marc:sysprefs/marc:syspref[@name='OPACResultsLibrary']" />
        ...
        <xsl:variable name="OPACItemLocation"
            select="marc:sysprefs/marc:syspref[@name='OPACItemLocation']" />
        <xsl:variable name="OPACTrackClicks"
            select="marc:sysprefs/marc:syspref[@name='TrackClicks']" />
        <xsl:variable name="BiblioDefaultView"
            select="marc:sysprefs/marc:syspref[@name='BiblioDefaultView']" />
        ...
        <xsl:variable name="physicalDescription">
            ...
            <xsl:variable name="check008-23">
                <xsl:if
                    test="$typeOf008='BK' or $typeOf008='MU' or $typeOf008='CR' or $typeOf008='MX'">
...
-->
            ...
            <xsl:if test="$cf007ss11 = 'c' and $cf007ss21 = 'c'">
                <img src="/opac-tmpl/lib/famfamfam/cd.png" alt="computer optical disc cartridge"
                    class="format" />
            </xsl:if>
            ...
            <xsl:if test="$cf007ss11 = 'c' and $cf007ss21 = 'r'"> available online <img
                    src="/opac-tmpl/lib/famfamfam/drive_web.png" alt="remote" class="format" />
            </xsl:if>
            ...
            <xsl:if test="$cf007ss11 = 'd' and $cf007ss21 = 'b'">
                <img src="/opac-tmpl/lib/famfamfam/world.png" alt="planetary or lunar globe"
                    class="format" />
            </xsl:if>
...
-->
                    ...
                    <xsl:for-each select="marc:subfield[contains('bcfghknps', @code)]">
                        <xsl:choose>
                            <xsl:when test="@code='h'">
                                <!--  13381 Span class around subfield h so it can be suppressed via
                                css -->
                            ...
                            <xsl:when test="@code='c'">
                                <!--  13381 Span class around subfield c so it can be suppressed via
                                css -->
        ...
        <!-- OpenURL -->
        <xsl:variable name="OPACShowOpenURL"
            select="marc:sysprefs/marc:syspref[@name='OPACShowOpenURL']" />
        ...
        <xsl:variable name="OpenURLResolverURL"
            select="marc:variables/marc:variable[@name='OpenURLResolverURL']" />
                        ...
                        <li>
                            <a>
                                <xsl:choose>
                                    ...
                                    <xsl:otherwise>
                                        <xsl:attribute name="href">
                                            /cgi-bin/koha/opac-search.pl?q=au:"<xsl:value-of
                                ...
                                <xsl:if
                                    test="(@tag=110 or @tag=710) and boolean(marc:subfield[@code='c' or @code='d' or @code='n' or @code='t'])">
                                ...
                                <xsl:if
                                    test="(@tag=111 or @tag=711) and boolean(marc:subfield[@code='c' or @code='d' or @code='g' or @code='n' or @code='t'])">
                                    <span class="titleportion">
                                        <xsl:choose>
                                            <xsl:when
                                ...
                                <xsl:if
                                    test="marc:subfield[@code='4' or @code='e'][not(parent::*[@tag=111])] or (self::*[@tag=111] and marc:subfield[@code='4' or @code='j'][. != ''])">
                                    <span class="relatorcode">
                                        ...
                                        <xsl:choose>
                                            ...
                                            <xsl:when test="marc:subfield[@code='e']">
                                                <xsl:for-each
                                                    select="marc:subfield[@code='e'][not(@tag=111) or not(@tag=711)]">
                                                    <xsl:value-of select="." />
                                                    <xsl:if test="position() != last()">, </xsl:if>
                                                </xsl:for-each>
                                            </xsl:when>
            ...
            <div class="results_summary type">
                <xsl:if test="$typeOf008!=''">
                    <span class="results_material_type">
                        ...
                        <xsl:choose>
                            <xsl:when test="$leader19='a'"><img
                                    src="/opac-tmpl/lib/famfamfam/ST.png" alt="Set"
                                    class="materialtype mt_icon_ST" /> Set</xsl:when>
                            <xsl:when test="$leader6='a'">
                                <xsl:choose>
                                    <xsl:when test="$leader7='c' or $leader7='d' or $leader7='m'"><img
                                            src="/opac-tmpl/lib/famfamfam/BK.png" alt="Text"
                                            class="materialtype mt_icon_BK" /> Text</xsl:when>
                                    <xsl:when test="$leader7='i' or $leader7='s'"><img
                                            src="/opac-tmpl/lib/famfamfam/SE.png"
                                            alt="Continuing resource"
                                            class="materialtype mt_icon_SE" /> Continuing resource</xsl:when>
                                    <xsl:when test="$leader7='a' or $leader7='b'"><img
                                            src="/opac-tmpl/lib/famfamfam/AR.png" alt="Article"
                                            class="materialtype mt_icon_AR" /> Article</xsl:when>
                                </xsl:choose>
                            </xsl:when>
                            <xsl:when test="$leader6='t'"><img src="/opac-tmpl/lib/famfamfam/BK.png"
                                    alt="Text" class="materialtype mt_icon_BK" /> Text</xsl:when>
                            <xsl:when test="$leader6='o'"><img src="/opac-tmpl/lib/famfamfam/MM.png"
                                    alt="Kit" class="materialtype mt_icon_MM" /> Kit</xsl:when>
                            <xsl:when test="$leader6='p'"><img src="/opac-tmpl/lib/famfamfam/MM.png"
                                    alt="Mixed materials" class="materialtype mt_icon_MM" />Mixed
                                materials</xsl:when>
                            <xsl:when test="$leader6='m'"><img src="/opac-tmpl/lib/famfamfam/CF.png"
                                    alt="Computer file" class="materialtype mt_icon_CF" /> Computer
                                file</xsl:when>
                            <xsl:when test="$leader6='e' or $leader6='f'"><img
                                    src="/opac-tmpl/lib/famfamfam/MP.png" alt="Map"
                                    class="materialtype mt_icon_MP" /> Map</xsl:when>
                            <xsl:when test="$leader6='g'"><img src="/opac-tmpl/lib/famfamfam/VM.png"
                                    alt="Film" class="materialtype mt_icon_VM" /> Film</xsl:when>
                            <xsl:when test="$leader6='k'"><img src="/opac-tmpl/lib/famfamfam/GR.png"
                                    alt="Picture" class="materialtype mt_icon_GR" /> Picture</xsl:when>
                            <xsl:when test="$leader6='r'"><img src="/opac-tmpl/lib/famfamfam/OB.png"
                                    alt="Object" class="materialtype mt_icon_OB" /> Object</xsl:when>
                            <xsl:when test="$leader6='c' or $leader6='d'"><img
                                    src="/opac-tmpl/lib/famfamfam/PR.png" alt="Score"
                                    class="materialtype mt_icon_PR" /> Score</xsl:when>
                            <xsl:when test="$leader6='i'"><img src="/opac-tmpl/lib/famfamfam/MU.png"
                                    alt="Sound" class="materialtype mt_icon_MU" /> Sound</xsl:when>
                            <xsl:when test="$leader6='j'"><img src="/opac-tmpl/lib/famfamfam/PR.png"
                                    alt="Music" class="materialtype mt_icon_PR" /> Music</xsl:when>
                        </xsl:choose>
                    </span>
                </xsl:if>
                ...
                <xsl:if
                    test="$controlField008-21 or $controlField008-22 or $controlField008-24 or $controlField008-26 or $controlField008-29 or $controlField008-34 or $controlField008-33 or $controlField008-30-31 or $controlField008-33">

                    <xsl:if test="$typeOf008='CR'">
                        <span class="results_typeofcontinuing">
                            <xsl:if
                                test="$controlField008-21 and $controlField008-21 !='|' and $controlField008-21 !=' '">
                                <span class="label">; Type of continuing resource: </span>
                            </xsl:if>
                            <xsl:choose>
                                <xsl:when test="$controlField008-21='d'">
                                    <img src="/opac-tmpl/lib/famfamfam/database.png" alt="database"
                                        class="format" />
                                </xsl:when>
                                ...
                                <xsl:when test="$controlField008-21='w'">
                                    <img src="/opac-tmpl/lib/famfamfam/world_link.png"
                                        alt="web site" class="format" />
                                </xsl:when>
                            </xsl:choose>
                        </span>
                    </xsl:if>
                        ...
                        <xsl:if test="contains($controlField008-24,'abcdefghijklmnopqrstvwxyz')">
                            <span class="results_natureofcontents">
                                ...
                                <xsl:choose>
                                    ...
                                    <xsl:when test="contains($controlField008-24,'b')"> bibliography <img
                                            src="/opac-tmpl/lib/famfamfam/text_list_bullets.png"
                                            alt="bibliography" class="natureofcontents" />
                                    </xsl:when>
                                    ...
                                    <xsl:when test="contains($controlField008-24,'t')">
                                        <img src="/opac-tmpl/lib/famfamfam/report.png"
                                            alt="technical report" class="natureofcontents" />
                                    </xsl:when>
                    ...
                    <xsl:if test="$typeOf008='CF'">
                        <span class="results_typeofcomp">
                            <xsl:if
                                test="$controlField008-26='a' or $controlField008-26='e' or $controlField008-26='f' or $controlField008-26='g'">
                            ...
                            <xsl:choose>
                                ...
                                <xsl:when test="$controlField008-26='f'">
                                    <img src="/opac-tmpl/lib/famfamfam/font.png" alt="font"
                                        class="format" />
                                </xsl:when>
                                <xsl:when test="$controlField008-26='g'">
                                    <img src="/opac-tmpl/lib/famfamfam/controller.png" alt="game"
                                        class="format" />
                                </xsl:when>
                            </xsl:choose>
                        </span>
                    ...
                    <xsl:if test="$typeOf008='BK'">
                        <span class="results_contents_literary">
                            <xsl:if
                                test="(substring($controlField008,25,1)='j') or (substring($controlField008,25,1)='1') or ($controlField008-34='a' or $controlField008-34='b' or $controlField008-34='c' or $controlField008-34='d')">
                                <span class="label">; Nature of contents: </span>
                            </xsl:if>
                            ...
                            <xsl:if
                                ...
                                <img src="/opac-tmpl/lib/famfamfam/user.png" alt="biography"
                                    class="natureofcontents" />
                            </xsl:if>

                            <xsl:if
                                test="$controlField008-33 and $controlField008-33!='|' and $controlField008-33!='u' and $controlField008-33!=' '">
                    ...
                    <xsl:if
                        test="$typeOf008='MU' and $controlField008-30-31 and $controlField008-30-31!='||' and $controlField008-30-31!='  '">
                        <span class="results_literaryform">
                            <span class="label">; Literary form: </span> <!-- Literary text for sound
                            recordings -->
                ...
                <xsl:if
                    test="($typeOf008='BK' or $typeOf008='CF' or $typeOf008='MU' or $typeOf008='VM') and ($controlField008-22='a' or $controlField008-22='b' or $controlField008-22='c' or $controlField008-22='d' or $controlField008-22='e' or $controlField008-22='g' or $controlField008-22='j' or $controlField008-22='f')">
                ...
                <xsl:text> </xsl:text> <!-- added blank space to fix font display problem, see Bug
                3671 -->
            </div>
        </xsl:if>
                ...
                <xsl:for-each select="marc:datafield[@tag=240]">
                    <a>
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:attribute name="href">/cgi-bin/koha/opac-search.pl?q=ti:<xsl:value-of
        ...
        <!-- RCM 987 -->
        <xsl:if test="marc:datafield[@tag=987]">
        ...
        <!-- Content Warning -->
        <xsl:variable name="ContentWarningField"
            select="marc:sysprefs/marc:syspref[@name='ContentWarningField']" />
                                ...
                                <a>
                                    <xsl:choose>
                                        <xsl:when test="$OPACTrackClicks='track'">
                                            <xsl:attribute name="href">
                                                /cgi-bin/koha/tracklinks.pl?uri=<xsl:value-of
                                                    select="str:encode-uri(marc:subfield[@code='u'], true())" />
                                                &amp;biblionumber=<xsl:value-of
                                    ...
                                    <xsl:choose>
                                        ...
                                        <xsl:when
                                            test="marc:subfield[@code='y' or @code='3' or @code='z']">
                                            <xsl:call-template name="subfieldSelect">
                        ...
                        <xsl:if test="$OPACURLOpenInNewWindow='1'">
                            <li>
                                <a target='_blank'>
                                    ...
                                    <xsl:choose>
                                        <xsl:when
                                            test="($Show856uAsImage='Results' or $Show856uAsImage='Both') and ($SubqText='img' or $SubqText='bmp' or $SubqText='cod' or $SubqText='gif' or $SubqText='ief' or $SubqText='jpe' or $SubqText='jpeg' or $SubqText='jpg' or $SubqText='jfif' or $SubqText='png' or $SubqText='svg' or $SubqText='tif' or $SubqText='tiff' or $SubqText='ras' or $SubqText='cmx' or $SubqText='ico' or $SubqText='pnm' or $SubqText='pbm' or $SubqText='pgm' or $SubqText='ppm' or $SubqText='rgb' or $SubqText='xbm' or $SubqText='xpm' or $SubqText='xwd')">
                                            <xsl:element name="img">
                                                <xsl:attribute name="src">
                                                    <xsl:value-of select="marc:subfield[@code='u']" />
                                                </xsl:attribute>
        ...
        <div class="results_summary availability">
            ...
            <xsl:choose>
                ...
                <xsl:when test="$itemcount=0">
                    <xsl:choose>
                        <xsl:when
                            test="string-length($AlternateHoldingsField)=3 and marc:datafield[@tag=$AlternateHoldingsField]">
                            <xsl:variable name="AlternateHoldingsCount"
                                select="count(marc:datafield[@tag=$AlternateHoldingsField])" />
                        <xsl:for-each
                                select="marc:datafield[@tag=$AlternateHoldingsField][1]">
                                <xsl:call-template name="subfieldSelect">
                                    <xsl:with-param name="codes">
                ...
                <xsl:otherwise>
                    ...
                    <xsl:if test="$sumAv>0">
                        <span class="available reallyavailable">
                            <span class="AvailabilityLabel">
                                <strong>
                                    <xsl:text>Items available for loan: </xsl:text>
                                </strong>
                            </span>
                            ...
                            <xsl:for-each
                                select="$available_items[not(items:resultbranch=preceding-sibling::*[items:status='available']/items:resultbranch)]">
                                ...
                                <span class="ItemSummary">
                                    <xsl:call-template name="listCallNumbers">
                                        <xsl:with-param name="items"
                                            select="$available_items[items:resultbranch=$currentbranch]" />
                                        ...
                                        <xsl:with-param name="OPACItemLocation"
                                            select="$OPACItemLocation" />
                                    </xsl:call-template>
                                </span>
                            </xsl:for-each>
                        </span>
                    ...
                    <!-- Availability part 2: ITEMS FOR REFERENCE (see also pref
                    Reference_NFL_Statuses) -->
                    <xsl:if test="$sumRef>0">
                        <span class="available reference">
                            <span class="AvailabilityLabel">
                                <strong>
                                    <xsl:text>Items available for reference: </xsl:text>
                                </strong>
                            </span>
                            ...
                            <xsl:for-each
                                select="$reference_items[not(items:resultbranch=preceding-sibling::*[items:status='reference']/items:resultbranch)]">
                                <xsl:sort select="items:resultbranch" />
                                <xsl:variable name="currentbranch" select="items:resultbranch" />
                                <xsl:for-each
                                    select="$reference_items[not(items:substatus=preceding-sibling::*[items:status='reference']/items:substatus)]">
                                    <xsl:sort select="items:substatus" />
                                    <xsl:variable name="current_substatus" select="items:substatus" />
                                    <span class="ItemSummary">
                                        <xsl:call-template name="listCallNumbers">
                                            <xsl:with-param name="items"
                                                select="$reference_items[items:resultbranch=$currentbranch and items:substatus=$current_substatus]" />
                                            <xsl:with-param name="max" select="$OPACResultsMaxItems" />
                                            <xsl:with-param name="status_text">
                                                <xsl:value-of select="concat($currentbranch,': ')" />
                                                <xsl:value-of
                                                    select="exsl:node-set($item_status_list)/status[@english=$current_substatus]|$current_substatus" />
                                                <!-- Note that value-of here picks string value of
                                                first node only from the union; keep in mind too
                                                that current_substatus may contain an authorised
                                                value -->
                                            </xsl:with-param>
                                            <xsl:with-param name="class_block"
                                                select="concat('notforloandesc_',$current_substatus)" />
                                            <xsl:with-param name="class_status"
                                                select="'ItemBranch'" />
                                            <xsl:with-param name="OPACItemLocation"
                                                select="$OPACItemLocation" />
                                        </xsl:call-template>
                    ...
                    <!-- Availability part 3: UNAVAILABLE ITEMS (see also pref
                    Reference_NFL_Statuses); status reallynotforloan or status other -->
                    <xsl:if test="number($sumAv+$sumRef) &lt; number($itemcount)">
                        <span class="unavailable">
                            ...
                            <xsl:variable name="unavailable_items"
                                select="key('item-by-status', 'reallynotforloan')|key('item-by-status', 'other')" />
                            <xsl:choose>
                                <xsl:when test="$OPACResultsUnavailableGroupingBy='branch'">
                                    <!-- First group by branch -->
                                    <xsl:for-each
                                        select="items:items/items:item[not(items:resultbranch=preceding-sibling::*/items:resultbranch)]">
                                        <xsl:sort select="items:resultbranch" />
                                        <xsl:variable name="currentbranch"
                                            select="items:resultbranch" />
                                        <span class="ItemSummary unavailable">
                                            <!-- Within same branch, group by substatus -->
                                            <xsl:for-each
                                                select="$unavailable_items[not(items:substatus=preceding-sibling::*[items:status='reallynotforloan' or items:status='other']/items:substatus)]">
                                                <xsl:sort select="items:substatus" />
                                                <xsl:variable name="current_substatus"
                                                    select="items:substatus" />
                                                <xsl:call-template name="listCallNumbers">
                                                    <xsl:with-param name="items"
                                                        select="$unavailable_items[items:resultbranch=$currentbranch and items:substatus=$current_substatus]" />
                                                    <xsl:with-param name="max"
                                                        select="$OPACResultsMaxItemsUnavailable" />
                                                    <xsl:with-param name="status_text">
                                                        <xsl:value-of select="$currentbranch" />
                                                        <xsl:text>: </xsl:text>
                                                        <xsl:value-of
                                                            select="exsl:node-set($item_status_list)/status[@english=$current_substatus]|$current_substatus" /><!--
                                                        See former comment on value-of and union -->
                                                    </xsl:with-param>
                                                    <xsl:with-param name="class_block"
                                                        select="concat('unavailable_',items:substatus)" />
                                                    <xsl:with-param name="class_status"
                                                        select="'ItemBranch'" />
                                                    <xsl:with-param name="OPACItemLocation"
                                                        select="$OPACItemLocation" />
                                ...
                                <xsl:when test="$OPACResultsUnavailableGroupingBy='substatus'">
                                    <!-- Only group by substatus, do not list branch names and
                                    individual call numbers. Meant for large consortia -->
                                    <xsl:for-each
                                        select="$unavailable_items[not(items:substatus=preceding-sibling::*[items:status='reallynotforloan' or items:status='other']/items:substatus)]">
                                        <xsl:sort select="items:substatus" />
                                        <xsl:variable name="current_substatus"
                                        ...
                                        <xsl:call-template name="listCallNumbers">
                                            <xsl:with-param name="items"
                                                select="$unavailable_items[items:substatus=$current_substatus]" />
                                            ...
                                            <xsl:with-param name="class_block"
                                                select="concat('unavailable_',$current_substatus)" />
                                            <xsl:with-param name="class_status"
                                                select="UnavailableSubstatus" />
                                        </xsl:call-template>
                                    </xsl:for-each>
                                </xsl:when>
                                <xsl:when test="$OPACResultsUnavailableGroupingBy='branchonly'">
                                    <!-- Only group by branch, do not list substatuses and
                                    ...
                                    <xsl:for-each
                                        ...
                                        <xsl:variable name="currentbranch"
                                            select="items:resultbranch" />
                                        <xsl:if
                                            test="count($unavailable_items[items:resultbranch=$currentbranch])>0">
                                            <span class="ItemSummary unavailable">
                                                <xsl:value-of select="$currentbranch" />
                                                <span class="unavailable">
                                                    <xsl:text> (</xsl:text>
                                                    <xsl:value-of
                                                        select="count($unavailable_items[items:resultbranch=$currentbranch])" />
                                                    <xsl:text>)</xsl:text>
                                                </span>
                                                <xsl:text>. </xsl:text>
                                            </span>
    ...
    <xsl:template name="listCallNumbers">
        ...
        <xsl:if test="count($items)>0">
            <span>
                <xsl:attribute name="class">
                    <xsl:value-of select="$class_block" />
                </xsl:attribute>
                <span>
                    <xsl:attribute name="class">
                        <xsl:value-of select="$class_status" />
                    </xsl:attribute>
                ...
                <xsl:text>)</xsl:text>
                <xsl:if
                    test="$max>0 and count($items[items:itemcallnumber!=''])>0 and $OPACItemLocation!='library'">
                    <span class="CallNumberAndLabel">
                        ...
                        <span class="CallNumber">
                            <xsl:for-each
                                select="$items[items:itemcallnumber!=''][position() &lt;= $max]">
                                ...
                                <xsl:if
                                    test="position()=last() and (count($items)>$max or count($items[items:itemcallnumber=''])>0)">
                                    <xsl:text>, ..</xsl:text>
                                </xsl:if>
                            </xsl:for-each>
                        </span>
